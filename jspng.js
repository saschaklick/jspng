...
var JSPNG = function(input, options){
	...
	this._fill  = new Buffer(8).fill(0xff);
	
	this.setInputMode(this.options['inputmode'] || 'r8g8b8a8');
...
JSPNG.prototype.setInputMode = function(inputMode){
	if(typeof inputMode !== 'string') throw new Error('input needs to be string');
	var re    = /([rgbaG\.])(\d*)/g;
	this._input = { bpp : 0 };
	while(match = re.exec(inputMode)){
		var channel = match[1], bpc = Number(match[2]) || 8;
		if(bpc !== 8)              throw new Error('only 8bit input channels are allowed at the moment [TODO]');
		if(channel in this._input) throw new Error('illegal multiple definition of same channel in input mode: "' + channel + '" at ' + match.index);
		switch(channel){
			case 'G':
				if(this._input.r || this._input.g || this._input.b) throw new Error('cannot not mix grayscale [G] and truecolor [rgb] channels in input mode');
			case 'r':
			case 'g':
			case 'b':
				if(this._input.G) throw new Error('cannot not mix grayscale [G] and truecolor [rgb] channels in input mode');
			case 'a':
...
JSPNG.prototype._tEXt = function(keyword, text){
	return this._chunk('tEXt', Buffer.concat([ new Buffer(keyword), new Buffer([0]), new Buffer(text) ]));
};

JSPNG.prototype._bKGD = function(){ return Buffer(0);
...
var CRC32 = function(buffer){
	...
	for (var n = 0; n < buffer.length; n++) {
		crc32 = crc32table[(crc32 ^ buffer[n]) & 0xff] ^ (crc32 >>> 8);
	}
